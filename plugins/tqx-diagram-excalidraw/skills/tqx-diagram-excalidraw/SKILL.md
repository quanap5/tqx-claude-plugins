...
This skill is the Excalidraw counterpart of the TwinQX brand. The same hard rules apply:

- **NO green** anywhere (Mộc khắc Thổ) — not for "success", not for status, not for code/JSON
  evidence text. Healthy/OK states use deep neutral (chocolate) or muted warm ink.
- **NO cool blue / teal / cyan** — that is the competitor space TwinQX differentiates from.
- **Warm temperature only** (terracotta / ochre / sienna / chocolate / cream).
- **Signature pair = terracotta `#A14B2B` + ochre `#D4A03E`** — lean on it for focal moments.
- **60-30-10 ratio** — mostly cream canvas + ink structure, ~10% terracotta/ochre accent.

## Customization

**All colors and brand-specific styles live in one file:** `references/color-palette.md` — the
TwinQX Terracotta Atlas palette. Read it before generating any diagram and use it as the single
source of truth for all color choices — shape fills, strokes, text colors, evidence artifact
backgrounds, everything. Everything else in this file is universal design methodology and
...
**Canvas background:** default to TwinQX cream `#F4ECD8` (light brand face). For control-room /
SCADA / live-operational diagrams, use the dark "Furnace Reactor" face — set
`"viewBackgroundColor": "#15110E"` and switch to the dark-face colors in `color-palette.md`.
...
If `uv` is unavailable, fall back to a venv: `python3 -m venv .venv && .venv/bin/pip install playwright && .venv/bin/playwright install chromium && .venv/bin/python render_excalidraw.py <file>`.
...
### First-Time Setup
If the render script hasn't been set up yet, from this skill's `references/` directory:
```bash
uv sync
uv run playwright install chromium
```
(`uv` is recommended; a plain `venv` + `pip install playwright` also works — see *How to Render*.)
